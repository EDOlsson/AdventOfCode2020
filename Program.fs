[<EntryPoint>]
let main argv =
    // let day1Part1 = Day01.CalculatePart1

    // printfn "Day 1 part 1 : %d" day1Part1
    // printfn "Day 1 part 2 : %d" <| Day01.CalculatePart2

    // printfn "Day 2 part 1 : %d" <| Day02.CalculatePart1
    // Day02.TestPart2 |> ignore
    // printfn "Day 2 part 2 : %d" <| Day02.CalculatePart2

    // printfn "Day 3 part 1 : %d" <| Day03.CalculatePart1
    // printfn "Day 3 part 2 (test) : %d" <| Day03.TestPart2
    // printfn "Day 3 part 2 : %d" <| Day03.CalculatePart2

    // printfn "Day 4 part 1 (test) : %d" <| Day04.TestPart1
    // printfn "Day 4 part 1 : %d" <| Day04.CalculatePart1
    // printfn "Day 4 part 2 (test) : %d" <| Day04.TestPart2
    // printfn "Day 4 part 2 : %d" <| Day04.CalculatePart2

    // printfn "Day 5 part 1 : %d" <| Day05.CalculatePart1
    // printfn "Day 5 part 2 : %d" <| Day05.CalculatePart2

    // printfn "Day 6 part 1 : %d" <| Day06.CalculatePart1
    printfn "Day 6 part 2 : %d" <| Day06.CalculatePart2
    0